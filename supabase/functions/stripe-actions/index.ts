...
async function handleApplyDiscount(stripe, data, accountId) {
  try {
    ...
    if (!couponToApply) return badRequest("Missing coupon_id and no promo_code in config");

    const updated = await stripe.subscriptions.update(subscription_id, {
      coupon: couponToApply,
    });

    return success(`Coupon ${couponToApply} applied to ${subscription_id}`, updated);
  } catch (err) {
