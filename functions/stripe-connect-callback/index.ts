...
serve(async (req) => {
  ...
  if (error) {
    return new Response(JSON.stringify({ error: "User denied Stripe access" }), {
      status: 400,
      headers: getCorsHeaders(),
    });
  }
  ...
  if (!code || !state) {
    return new Response(JSON.stringify({ error: "Missing code or state from Stripe callback" }), {
      status: 400,
      headers: getCorsHeaders(),
    });
  }
  ...
  if (!insertRes.ok) {
    ...
    return new Response(JSON.stringify({ error: "Failed to store credentials", details: insertData }), {
      status: 500,
      headers: getCorsHeaders(),
    });
  }
